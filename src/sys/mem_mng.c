...
/* free_mem_blk_t data structure defines parameters of the Free Block */
typedef struct free_mem_blk {
    struct free_mem_blk	*next;	/* Pointer to the next memory block */
...
/* busy_mem_blk_t data structure defines parameters of the Busy Block  */
typedef struct busy_mem_blk {
    struct busy_mem_blk	*next;	/* Pointer to the next memory block */
...
typedef struct mem_mng {
    ...
    busy_mem_blk_t	*busy_blks;		/* List of busy blocks (Busy list) */
    free_mem_blk_t	*free_blks[MEM_MNG_MAX_ALIGNMENT + 1];
